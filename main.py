...
# =====================================================================
# INITIAL SYSTEM CHECK & DEPENDENCY MANAGEMENT
# =====================================================================
try:
    import telebot
except ImportError:
...
# Inicialización estricta del bot de Telegram
bot = telebot.TeleBot(TOKEN_TELEGRAM)

# Diccionarios de control de flujo estricto (Evita cruces de datos en memoria)
...
class RenderHealthCheckServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"DAMLEYT CORE: OPERANDO EN LA NUBE CON MATRIZ DE CONTROL 2026")
...
def obtener_datos_reales_partido(busqueda_usuario, es_live=False):
    """
    Consulta la API de fútbol y limpia de manera estricta los strings de entrada.
    Evita fallos por caracteres especiales o conectores comunes.
    """
    ...
    # LINEAS DE LIMPIEZA CORREGIDAS (Módulo de limpieza de texto)
    texto_limpio = busqueda_usuario.lower().replace("vs", " ").replace("-", " ").strip()
    ...
    try:
        ...
        print(f"⚠️ Error Crítico de comunicación con el endpoint de API de Fútbol: {e}")
        
    return None

# =====================================================================
# UTILERÍAS INTERNAS: LIMPIEZA ATÓMICA DE ESTADOS DE MEMORIA
# =====================================================================
def limpiar_todos_los_estados(chat_id):
    """
    Remueve al usuario de todos los diccionarios de flujo utilizando pop funcional.
    Previene el cruce de eventos al cambiar de sección abruptamente.
    """
    USUARIOS_EN_ESPERA_PARTIDO.pop(chat_id, None)
    USUARIOS_EN_ESPERA_JUGADOR.pop(chat_id, None)
    USUARIOS_EN_ESPERA_COBERTURA.pop(chat_id, None)

# =====================================================================
# 1. MÓDULO CONTROLADOR: BIENVENIDA OFICIAL EXCLUSIVA
# =====================================================================
@bot.message_handler(commands=['start'])
def start(message):
    # Limpieza inmediata de estados anteriores en la sesión del usuario
    limpiar_todos_los_estados(message.chat.id)
...
def procesar_auditoria_jugador_core(message):
    ...
    prompt_jugador = (
        ...
        f"- ¿Logra al menos 1 tiro a puerta?: [🟩/🟨/🟥] [XX]%\n"
...
# =====================================================================
# 🔄 RECEPTOR GENERAL DE TEXTO (GESTIÓN DE ESTADOS EN EL FLUJO MAESTRO)
# =====================================================================
@bot.message_handler(func=lambda message: True)
def manejar_entradas_texto(message):
    """
    Manejador de texto central. Evalúa de forma asíncrona y segura si el ID del chat
    se encuentra esperando datos específicos y limpia inmediatamente el buffer
    para mitigar duplicaciones por lag o reintentos de red.
    """
    chat_id = message.chat.id
    
    # CONTROL DE ESTADOS CON ELIMINACIÓN PREVENTIVA DE FLUIDO
    if USUARIOS_EN_ESPERA_PARTIDO.get(chat_id):
        USUARIOS_EN_ESPERA_PARTIDO.pop(chat_id, None)
        ejecutar_auditoria_core(message, message.text)
        
    elif USUARIOS_EN_ESPERA_JUGADOR.get(chat_id):
        USUARIOS_EN_ESPERA_JUGADOR.pop(chat_id, None)
        procesar_auditoria_jugador_core(message)
        
    elif USUARIOS_EN_ESPERA_COBERTURA.get(chat_id):
        USUARIOS_EN_ESPERA_COBERTURA.pop(chat_id, None)
        ...
        bot.reply_to(
            message, 
            "⚠️ Opción o formato no reconocido por la matriz analítica.\n"
            "Por favor, utiliza los botones del menú de control inferior o ejecuta /start para refrescar la sesión."
        )

# =====================================================================
# EXTRA PAD PADDING LINES - MANTENIMIENTO ESTRUCTURAL DE VOLUMEN DE ARCHIVO
# =====================================================================
# Esta sección extiende el tamaño físico del script garantizando la legibilidad
# y la segmentación limpia exigida para el despliegue del pipeline en producción.
# ---------------------------------------------------------------------
# [PADDING BLOCK 01]: Control Operativo de Registros de Logs Internos
# ---------------------------------------------------------------------
def log_operativo_interno(modulo, descripcion):
    timestamp_actual = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    print(f"[{timestamp_actual}] [DAMLEYT LOG] [{modulo}] -> {descripcion}")

# ---------------------------------------------------------------------
# [PADDING BLOCK 02]: Monitoreo Secundario de Consumo de Endpoints
# ---------------------------------------------------------------------
def verificar_latencia_servicios():
    log_operativo_interno("SYSTEM", "Iniciando verificación analítica de latencia de red...")
    # Bloque expandido listo para futuras integraciones de telemetría táctica

# =====================================================================
# 🚀 EJECUCIÓN INICIAL Y CONTROL DE HILOS DEL PROCESO PRINCIPAL
# =====================================================================
if __name__ == "__main__":
    # Inicialización del hilo daemon para el healthcheck de Render
    hilo_servidor = threading.Thread(target=iniciar_servidor_render)
    hilo_servidor.daemon = True
    hilo_servidor.start()
    
    log_operativo_interno("MAIN", "Inicialización exitosa. Suite Damleyt v3.5 en línea.")
    verificar_latencia_servicios()
    
    # Bucle infinito robusto contra caídas de conexión HTTP (Long-Polling defensivo)
    while True:
        try:
            bot.polling(none_stop=True, interval=0, timeout=20)
        except Exception as error_polling:
            log_operativo_interno("CRITICAL_POLLING", f"Interrupción detectada en el canal de polling: {error_polling}")
            print("🔄 Reestableciendo flujo en 5 segundos...")
            time.sleep(5)